var unique = require('array-unique'),
  element_route_json = require('./element_route.json');

module.exports = {

  /*
  feature: find the root from driver's element
  parameter: [ [integer,integer(optional),integer(optional)]...] * require 3 set of array in array
  parameter rule
    0 <- fire
    1 <- water
    2 <- wind
  ...
  return:
    [
      {step1:<element name>, step2:<element name>, step3:<element name>},
      ....,
      ....,
      ....
    ]
  */
  cal_v2: function(driver1, driver2, driver3){

    var err = "Please insert corrent format as 3 set of array",
    fire = 0,
    water = 1,
    wind = 2,
    earth = 3,
    ...
    if(arguments.length != 3){
      console.log("null input error");
      return err;
    }

    //check for wrong input error
    if(driver1.length <= 0 || driver2.length <= 0 || driver3.length <= 0){
      console.log("wong input error");
      return err;
    }
    else if(driver1.length > 0 || driver2.length > 0 || driver3.length > 0){
      console.log("pass");
    }
